...
static int ZigZagArray[64] = { 0, 1, 5, 6, 14, 15, 27, 28, 2, 4, 7, 13, 16, 26,
		29, 42, 3, 8, 12, 17, 25, 30, 41, 43, 9, 11, 18, 24, 31, 40, 44, 53, 10,
		19, 23, 32, 39, 45, 52, 54, 20, 22, 33, 38, 46, 51, 55, 60, 21, 34, 37,
...
inline void DecodeSingleBlock(stComponent *comp, unsigned char *outputBuf,
		int stride) {
...
//
/***************************************************************************/
inline void WriteBMP24(const char* szBmpFileName, int Width, int Height,
		unsigned char* RGB) {
#pragma pack(1)
...
#pragma pack()
	...
	bh.bmtype[0] = 'B';
	bh.bmtype[1] = 'M';
	bh.iFileSize = (Width * Height * 3) + (Height * iNumPaddedBytes)
			+ sizeof(bh);
	...
	for (int y = Height - 1; y >= 0; y--) {
		for (int x = 0; x < Width; x++) {
			int i = (x + (Width) * y) * 3;
			unsigned int rgbpix = (RGB[i] << 16) | (RGB[i + 1] << 8)
					| (RGB[i + 2] << 0);
...
/***************************************************************************/
inline void BuildHuffmanTable(const unsigned char *bits,
		const unsigned char *stream, stHuffmanTable *HT) {
	for (int j = 1; j <= 16; j++) {
...
inline void BuildQuantizationTable(float *qtable,
...
inline int ParseSOS(stJpegData *jdata, const unsigned char *stream) {
	...
	cout << endl << "> SOS marker\n";

	if (nr_components != 3) {
		cout << endl << "Error - We only support YCbCr image\n";
...
inline int ParseJFIF(stJpegData *jdata, const unsigned char *stream) {
	...
	if (!dht_marker_found) {
		cout << endl << "ERROR> No Huffman table loaded\n";

	}

	return 0;

	bogus_jpeg_format: cout << endl << "ERROR> Bogus jpeg format\n";
...
inline int JpegParseHeader(stJpegData *jdata, const unsigned char *buf,
		unsigned int size) {
	// Identify the file
	if ((buf[0] != 0xFF) || (buf[1] != SOI)) {
		cout << endl << "Not a JPG file ?\n";
	...
	cout << endl << "-|- File thinks its size is: " << fileSize << " bytes\n";
...
inline void JpegGetImageSize(stJpegData *jdata, unsigned int *width,
		unsigned int *height) {
	*width = jdata->m_width;
...
bool IsInHuffmanCodes(int code, int numCodeBits, int numBlocks, stBlock* blocks,
		int* outValue) {
...
void DumpHufCodes(stHuffmanTable* table) {
	...
	for (int i = 0; i < table->m_numBlocks; i++) {
		cout << endl << "%03d\t [%s]\n" << i << " "
				<< IntToBinary(table->m_blocks[i].code,
						table->m_blocks[i].length);
	}
	cout << endl;
...
//	DumpHufCodes(c->m_acTable);
	...
	for (int k = 1; k < 16; k++) {
		...
		if (IsInHuffmanCodes(code, k, c->m_dcTable->m_numBlocks,
				c->m_dcTable->m_blocks, &decodedValue)) {
			...
			if (numDataBits == 0) {
				...
				if (jdata->m_restart_interval > 0)
					if (jdata->m_stream[0] == 0xff
							&& jdata->m_stream[1] != 0x00) {
						jdata->m_stream += 2;
						g_reservoir = 0;
						g_nbits_in_reservoir = 0;
					}
	...
	if (!found) {
		cout << endl
				<< ("-|- ##ERROR## We have a *serious* error, unable to find huffman code\n");
	...
	while ((nr <= 63) && (!EOB_found)) {
		...
		for (k = 1; k <= 16; k++) {
			...
			// Check if its one of our huffman codes
			if (IsInHuffmanCodes(code, k, c->m_acTable->m_numBlocks,
					c->m_acTable->m_blocks, &decodedValue)) {
				...
				if (size_val == 0) { // RLE
					...
					if (nr > 63) {
						cout << endl << ("-|- ##ERROR## Huffman Decoding\n");
...
inline void YCrCB_to_RGB24_Block8x8(stJpegData *jdata, int w, int h, int imgx,
		int imgy, int imgw, int imgh) {
...
inline void DecodeMCU(stJpegData *jdata, int w, int h) {
	...
	for (int y = 0; y < h; y++) {
		for (int x = 0; x < w; x++) {
			...
			DecodeSingleBlock(&jdata->m_component_info[cY], &jdata->m_Y[offset],
					stride);
		}
	}
...
inline int JpegDecode(stJpegData *jdata) {
	...
	for (int y = 0; y < (int) jdata->m_height; y += ystride_by_mcu) {
		for (int x = 0; x < (int) jdata->m_width; x += xstride_by_mcu) {
			jdata->m_colourspace = jdata->m_rgb + x * 3
					+ (y * jdata->m_width * 3);
			...
			YCrCB_to_RGB24_Block8x8(jdata, hFactor, vFactor, x, y,
					jdata->m_width, jdata->m_height);
		}
	}
...
//
/***************************************************************************/
unsigned char* readFile(char* jpgFileName, unsigned int &lengthOfFile) {
	FILE *fp;
	lengthOfFile = 0;
	unsigned char *buf;
