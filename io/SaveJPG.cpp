...
//should be<=7 and >=0
static WORD mask[16] = { 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048,
...
void load_bitmap(char *bitmap_name, WORD *Ximage_original,
		WORD *Yimage_original) {
	WORD Xdiv8, Ydiv8;
	...
	*Yimage_original = Yimage; //Keep the old dimensions
	...
	for (nrline_up = Yimage - 1, nrline_dn = 0; nrline_up > nrline_dn;
			nrline_up--, nrline_dn++) {
		memcpy(tmpline, RGB_buffer + nrline_up * Ximage, dimline);
		memcpy(RGB_buffer + nrline_up * Ximage, RGB_buffer + nrline_dn * Ximage,
				dimline);
...
/*void saveJPGFromBuffer(unsigned char *buffer, int width, int height,
		char* filename) {
	bitstring fillbits;
