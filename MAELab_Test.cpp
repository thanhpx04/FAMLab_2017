...
void extractLandmarkPatch(string image_file, string landmark_file, int width,
	...
	for (int i = 0; i < landmarks.size(); i++) {
		...
		Matrix<int> patch = matImage.getGrayMatrix()->extractPatch(width,
				height, pi.getY(), pi.getX(), 0);
...
void calculateSIFT(string image_file, string lm_file, int patchsize, string save_folder)
{
	Image matImage(image_file);
